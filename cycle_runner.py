...
def export_portfolio(db_path: Path, out_dir: Path) -> tuple[Path, Path]:
...
def main() -> None:
    ...
    json_path, csv_path = export_portfolio(db_path, out_dir)
    latest_dir = Path("data/paper_cycles/latest")
    latest_dir.mkdir(parents=True, exist_ok=True)
    latest_json, latest_csv = export_portfolio(db_path, latest_dir)
    print(json.dumps({
        "db_path": str(db_path),
