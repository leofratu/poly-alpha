...
| `api/server.py` | Stdlib-only read-only JSON API: `DataProvider`, `StaticProvider`, `default_provider`, `create_server`; endpoints `/health`, `/markets`, `/research`, `/risk`, `/compare`, `/overview`, `/validation`, `/curves`, `/calibration`, `/stress`, `/allocate`, `/run`, `/experiments` (with `?verify=1`), `/sources`, plus a read-only HTML dashboard at `/`. |
