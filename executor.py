...
def fetch_l2_markets(limit=2000):
    ...
    req = urllib.request.Request(url, headers={"User-Agent": "PolyAlpha/4.0"})
    try:
...
def evaluate_risk_ai(markets):
    if not GEMINI_API_KEY:
        console.print("[yellow]GEMINI_API_KEY not found. Skipping AI semantic risk evaluation.[/yellow]")
        for m in markets:
            m["ai_risk"] = 0.05
        return markets

    console.print(f"[cyan]Sending {len(markets)} markets to Gemini AI for tail-risk analysis...[/cyan]")
    ...
    try:
        with urllib.request.urlopen(req) as resp:
            ...
            for i, m in enumerate(markets):
                m["ai_risk"] = float(risk_scores[i]) if i < len(risk_scores) else 0.05
    except Exception as e:
        console.print(f"[red]AI Evaluation Failed: {e}. Defaulting to baseline risk.[/red]")
        for m in markets:
            m["ai_risk"] = 0.05
...
def run_executor(capital=100000.0):
    ...
    console.print(f"[green]AI approved {len(safe_markets)} markets as pure retail bias (Tail Risk <= 15%).[/green]\n")
    ...
    console.print(f"[bold]TOTAL CAPITAL ALLOCATED:[/bold] ${total_deployed:,.2f} / ${capital:,.2f}")
    console.print("[bold yellow]Action:[/bold yellow] To execute live on Polygon, integrate Ethers.py with Polymarket CTF Exchange addresses and sign the payload with your private key.")
