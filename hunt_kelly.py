    ...
    for event in events:
        for m in event.get("markets", []):
            ...
            try:
                ...
                liquidity = float(m.get("liquidity", volume * 0.05)) # Approximate BBO depth if liquidity not provided
                ...
                if yes_price >= 0.01 and yes_price <= 0.20:
                    ...
                    # f* = (bp - q) / b
                    # p = True probability of winning (we assume P-Measure from Reichenbach is ~98% for these favorites)
                    # b = Net fractional odds received on the wager ($1 / no_price) - 1
                    # q = Probability of losing (1 - p)
                    
                    b = (1.0 / no_price) - 1.0
                    p = 0.98 # Conservative True P-Measure (98% probability "No" wins)
                    q_loss = 1.0 - p
...
def size_portfolio(total_portfolio_usd=100000.0):
    ...
    console.print(f"[bold cyan]Kelly Criterion Capital Allocation | Portfolio: ${total_portfolio_usd:,.2f}[/bold cyan]\n")
    ...
    for m in markets[:10]:
        ...
        remaining_capital = total_portfolio_usd - total_allocated
        ...
        expected_profit += actual_allocation * m["roi"]
    ...
    console.print(f"[bold]Expected Absolute Profit (48-72h):[/bold] [bold green]+${expected_profit:,.2f}[/bold green]")
